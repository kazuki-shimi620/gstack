...
- [ ] Google Sheets metadata read、HTTP retry、Migration Operation mapping、`create_model`／`add_column`／`rename_column` write契約はD-040／D-042／D-050／D-053／D-056／D-082で確定し、各adapterを実装済み。残りのwrite Operationごとにデータ保持、rollback、idempotencyを確定する。
