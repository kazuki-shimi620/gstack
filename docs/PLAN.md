...
Google Provider Status: `@gstack/provider-google` package、公式Manifest、strictな非secret configuration、offline validation、注入可能なGoogle Workspace Gateway、safe health境界を実装済み。Apps Script API実行との互換性からMVP認証を利用者OAuthに限定し、operation別の最小scope requestを定義した。未実装のGoogle Sheets Migration操作は全て`unsupported`である。次はcredential payload／refreshをSecret Resolver adapter境界として確定し、Google Sheets Database capabilityをread-only metadata境界から実装する。Migration Apply、Apps Script生成／Deploy、OAuth token保存はまだ実装しない。
