...
Project Config Status: optionalな`providers.<name>.enabled／configuration`をProvider非依存のopaque JSON-compatible configとして読込み、name順のimmutable arrayへ正規化する。Config packageは具体Providerを知らず、secret値／package参照を持たない。`@gstack/runtime` composition rootがenabledな公式Google ProviderをRegistry／Inspectionへ接続し、CLI／MCPの実entry pointから利用する。disabled／未知Providerを暗黙loadせず、Environment Secret Resolverは安全な変数名だけを解決する。CLIにはProvider list／info／validate／health Read commandを追加し、install／remove／useは未実装のまま維持する。次はGoogle Migration capability mappingの設計へ進む。
