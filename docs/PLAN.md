...
Google Provider Status: `@gstack/provider-google` package、公式Manifest、strictな非secret configuration、offline validation、注入可能なGoogle Workspace Gateway、safe health境界を実装済み。未実装のGoogle Sheets Migration操作は全て`unsupported`である。次はGatewayの認証／health adapter契約を具体化し、Google Sheets Database capabilityをread-only metadata境界から実装する。Migration Apply、Apps Script生成／Deploy、OAuth token保存はまだ実装しない。
