...
Status: 実装中。Provider非依存のOperation／Plan、pure Diff、Migration File／checksum／strict YAML、History状態遷移、checksum付きApplication Model snapshot、History Storage port／Repository、Operation単位のcapability評価を実装済み。次はMigration Read APIへ進む。Providerを実際に変更するApply／Rollbackは引き続き未実装とする。
