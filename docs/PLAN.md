...
Project Config Status: optionalな`providers.<name>.enabled／configuration`をProvider非依存のopaque JSON-compatible configとして読込み、name順のimmutable arrayへ正規化する。Config packageは具体Providerを知らず、secret値／package参照を持たない。次は標準composition rootがenabledな公式ProviderをRegistry／Inspectionへ接続する。
