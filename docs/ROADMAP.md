...
Status: Provider package、Manifest、strict configuration、offline validation、OAuth／HTTP safety、Google Sheets、Drive folder、Apps Script projectのread-only metadata REST adapter、実Sheets接続を使うread-only healthまで実装済み。各Capabilityのwrite、Migration Apply、Deployは未実装。
