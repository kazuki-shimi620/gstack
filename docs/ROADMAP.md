...
Status: Stable Release監査を開始。Node.js 24、`npm ci`、repository全checkを実行する最小権限GitHub Actions CIを追加済み。D-092の`gstack init`、D-093の`schema init`、D-094のProvider非依存`migration create`を実装し、生成Projectをbuilt CLIでinit、Schema作成、semantic validation、初回Migration File作成までcontract testした。sample projectのGeneration／Deploy dry-run、公開package metadata／pack検証、1.0互換性baselineは未完了であり、実registry publishは行わない。
