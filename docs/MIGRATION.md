...
Provider非依存のRollback Plannerは、検証済みMigration File、完了Operation数、直前のApplication Model snapshotを入力とするpure APIとして実装済みである。通常PlanのOperation ID sortを適用せず、forward実行順の逆順を保持する。完了範囲に`reversible: false`が含まれる場合は`MIGRATION_IRREVERSIBLE`を返す。D-090に従う実行Engineは専用History進捗、fingerprint／destructive承認、Provider lock、Operation単位の永続化、明示resumeを担い、Google RuntimeとCLIへ接続する。MCP Rollbackや暗黙Rollbackは提供しない。
